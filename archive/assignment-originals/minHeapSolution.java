...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	public Decreaser<T> insert(T thing) {
		...
		array[size] = ans;  //Takes the value to be put in the heap and inserts it into
							//the last location in the aray
		//System.out.println("The first value and location is " + ans);
		decrease(size);  //Pass the size to the decrease method
	...
	void decrease(int loc) {
		...
		//
		if(loc<=1){//If the heap is so small that it does not have parent
			return;
		}
		else{
			T child = array[loc].getValue(); //Assigns the new value as a child
			T parent = array[loc/2].getValue(); //Assigns the value up the heap as the parent
			if(child.compareTo(parent) < 0){ //Compares the two to see if one needs to be relocated
				relocate(loc,loc/2); //If a relocate needs to take place the below method is called
				decrease(loc/2); //The recursive call is done to continue up the heap until the root is reached
				//System.out.println("This is loc " + loc);
				//System.out.println("This is loc/2 " + loc/2);
			}
		}
	}
	/*
	 * @param child takes in the child location for swapping
	 * @param parent takes in the parent location for swapping
	 */
	void relocate(int child, int parent){
		Decreaser<T> placeholder = array[child]; //takes in the child to be moved to a temp object
		array[child] = array[parent]; // moves parent to child location
		array[parent] = placeholder; // moves child to parent
		array[child].loc = child; //Change the location of the loc for the next decrease run with regards to that child
		array[parent].loc = parent; //Change the location of the loc for the next decrease run with regards to that parent
	}
