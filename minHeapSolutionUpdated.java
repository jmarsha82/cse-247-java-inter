...
//Placement of heaps and heapify help from Steven Corman
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void decrease(int loc) {
		...
		if(loc <= 1) {
			return; // Base Case is that if the objects location is at the top of the tree
		}
		else {
			T child = array[loc].getValue();//takes numeric value of object
			T parent = array[loc/2].getValue();//takes numeric value of object
			if(parent.compareTo(child) > 0) {//compares the objects to see which numeric value is larger
				ticker.tick();
				move(loc, loc/2); //sends a loc to relocate to do a swap
				decrease(loc/2); //recursive call to move up the tree
			}
		}
	}

	void move(int from, int to){
		Decreaser<T> placeholder = array[from]; //creates a placeholder for the move
		array[from] = array[to]; //Does the move
		array[to] = placeholder; //Does the move
		array[from].loc = from; //Updates location of objects
		array[to].loc = to; //Updates location of objects
	}
	
	public int getRightChild(int where) {
		return (where * 2) + 1;  //2 * i + 1 is the right child location given the parent
	}
	
	private int getLeftChild(int where) {
		return where * 2;  //2 * i is the left child location given the parent
	}
	...
	public T extractMin() {
		...
		array[1] = array[size];//Takes the largest value in the tree and moves it to the top
		array[1].loc = 1;//Updates the loc for that value
		array[size] = null;//Fills in the empty space at the end of the array with null
		size -= 1;//Changes the size of array to account for this move
		heapify(1);//Heapify for that value will move it back down to where it goes but sort the tree in the process
		return ans;
	}
	...
	private void heapify(int where) {
		//left Child is where * 2
		//right child is where * 2 + 1
		T parent;
		T leftChildT;
		T rightChildT;
		if((where * 2) + 1<= size) {//Checks to see if the right child is at the end of the array
			ticker.tick();
			parent = array[where].getValue(); //create parent node and assign what is at the where location to it
			leftChildT = array[where * 2].getValue(); //create left child node
			rightChildT = array[(where * 2) + 1].getValue(); //create right child node
			if(parent.compareTo(leftChildT) < 0 && parent.compareTo(rightChildT) < 0) {
				ticker.tick();
				return; //Base Case.  If everything is where it should be
			}
			if(rightChildT.compareTo(leftChildT) > 0) {
				ticker.tick();
				move(where * 2, where); //If the left child is greater than the right relocate and start over
				heapify(where * 2);

			}
			else {
				ticker.tick();
				move((where * 2) + 1, where); //If the right child is greater than the parent relocate and start over
				heapify((where * 2) + 1);			
			}

		}
		else if(where * 2 <= size){ //Checks to see if the left child is at the end of the array in case the first if doesn't catch the end
			ticker.tick();
			parent = array[where].getValue(); //create parent node and assign what is at the where location to it
			leftChildT = array[where * 2].getValue(); //create left child node
			if(parent.compareTo(leftChildT) > 0) {
				ticker.tick();
				move(where * 2, where); //If the left child is greater than the parent relocate and start over
				heapify(where * 2);
			}
		}
	}
